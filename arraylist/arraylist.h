...
namespace ctools
{
    ...
    template <class T>
    bool ctools::ArrayList<T>::operator==(const ArrayList &two)
    {
        if(top!=two.top) return false;
        if(top>0){
            for(int i=0;i<top;i++){
                if(arr[i]!=two.arr[i]) return false;
            }
        }
        return true;
    }
    ...
    template <class T>
    ArrayList<T> &ArrayList<T>::operator=(const T *input)
    {
        if(arr!=NULL){
            delete[] arr;
        }
        int langth=sizeof(input)/sizeof(T);
        arr=new T[langth+1];
        memcpy(arr,input,langth);
    }
